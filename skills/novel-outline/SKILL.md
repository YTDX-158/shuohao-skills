...
一次问完，别轮流盘问。三件必问（或已锁定）+ 两件给默认值待确认：

| 参数 | 处理 |
| --- | --- |
| **总集数 × 单集时长** | **必问**，没有合理默认 |
| **题材** | **必问**，决定爽点类型，猜错整份废 |
| **视频比例** | **必问/沿用**——默认 9:16 竖屏（可改 16:9 横屏）。比例影响下游分镜图 + 出图包（novel-storyboard 分镜图、novel-assets 出图都按它走），outline 阶段定掉，别等跑完分镜才发现。**定完把比例写进 outline.json 顶层 `ratio`**（如 `"ratio": "9:16"`）——这是比例链的源头固化点，下游 storyboard/novel-assets 从它读，别只留在对话里。见规则 feedback_pipeline_preflight_params |
