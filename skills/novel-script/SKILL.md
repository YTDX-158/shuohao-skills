...
核心要求都在 script-pass.md 里，最重的四条：**动作节拍只写常见动作**（挑担上船、搭手卸担这种 AI 见过千万次的；伸篙一挡、睫毛颤这种精巧动作生成必崩）；**时长预算先于一切**（三分钟一集约 50 个节拍，写完自己跑一遍 validate 看秒数）；台词口语、单句一口气、**谁的话像谁**（有 cast.json 就吃角色的性情与说话方式）；**每集第 1 拍冷开场给钩子的具象**（`hookBeat` 认领，门查位置），结尾一拍必须是悬念。**主场景别全程一个情绪**——长场留常态→突变拍，给下游 art 光照反推留原料（见 script-pass 硬规则 9）。
...
### Step 3.5 — 顾问软审（可选，高级功能）

**validate 通过后、故事板之前**，用户可以说"顾问审一遍"——让 AI 当独立审稿人，从**人物动机 / 情感铺垫 / 逻辑漏洞 / 伏笔缺失 / 爽感节奏**五角度软性审一遍剧本，输出建议清单 txt。**不卡门、不强制改，只给参考。**

**定位**：重要项目 / 剧本卡壳 / 感觉"哪里不对劲但说不出来"时才用。**validate 全部通过后脚本会自动打印一行软审提示**（"说顾问审一遍"），用户看到就点跑；不强制。**时机红线**：只在剧本定稿前跑——进入 storyboard / 出图后剧本冻结，改 = 下游重跑。

读 `{baseDir}/references/adviser-pass.md`，照它的执行细则做：独立审稿人视角（子代理优先/人格切换兜底）、五角度、证据规则（局部带原文、整体带落点防胡诌）、归属层（剧本/outline/characters，上游问题回上游改）、分级、定位到"第X集·场Y·第Z拍"、输出 `<剧名>-script-adviser.txt`（top3 优先 + 只能改1处提示 + 一句话整体印象）。

**闭环**：用户挑最多 3 条 → 我改 script.json（**定向补丁，不是重跑**，防"重跑=完全重新生成"误伤）→ 重新 validate → 下游 storyboard 相关段重跑。归属 outline/characters 的提示回上游改。收敛：每轮最多改 3 条，改完这轮结束，主动说"再审"才再审。开头明说代价"采纳可能触发下游重跑"。
