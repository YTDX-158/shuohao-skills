...
3. **光照状态覆盖"场景在戏里的所有状态变化"。**这个场景在出现的那些集里经历了什么——不只是时段/天气，**还包括戏的状态变化**（教室的"日常→启灵仪式诡气弥漫"、牢房的"安静→审讯"，同一个场景换一种戏状态就是换一种光）。每种状态各写一条完整提示词，别让后端的剧本用上 art 没有登记的光。**宁缺毋滥的边界**：别凭空写一套「白天/夜晚/黄昏」全家桶，只写这个场景在戏里**真的会经历**的状态——从出现集、承载爽点、人物小传里的场景戏反推。

**主场景配常态档**：主场景（`primary`）即使戏里只有事件状态（启灵/灾难），也补一条**常态档**——场景不带事件时的样子（教室=正常上课、街头=日常），作对照也备用（跨集/后续集可能用）。**无日常状态可言的场景（灾难蒙太奇/纯仪式）不强求。**
