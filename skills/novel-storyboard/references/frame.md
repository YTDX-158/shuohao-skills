...
一个镜一张 16:9 关键帧，**每段一个文件夹**：`<段号>/f<切序>.png`（f1 = 该镜分镜图），`seedancePrompt` 由 `export` 写成同文件夹里的 `prompt.md`（每镜一条，与 f 图一一对应）——每镜一图一提示词，独立生成（一镜一视频），构图由图控制，不靠文字赌。**它是资产合成，不是凭空画**：场景、角色、道具长什么样已经由上游设定图定死，分镜图只负责把它们按这一格的构图摆进同一帧。
