...
| [**novel-assets**](skills/novel-assets) | 出图闭环（第 6 阶段）：把 cast/art/storyboard 里已写好的 image.sheet 与 frame 提示词打包成「发 GPT 的任务文本」，图回来归位，report 补图。不写提示词、不生成图（GPT 网页版手动出图）、不改 JSON。run / place / render 三命令 |
